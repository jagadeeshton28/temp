...
class Products extends React.Component {
    ...
    handleSearchChange = (e) => {
        ...
        if (validator.isEmail(searchEmail) && e.target.value!=null) {
    ...
    handleUpdateBid(productId) {
        let currentProductData = this.props.productList.filter(currentData => currentData.productId === productId);
        let currentBidData = this.props.productBidList.filter(productBid => productBid.productId === productId && productBid.email === localStorage.getItem('searchEmail'));
        ...
        return (
            <div>
                {(this.state.userType === "seller") ?
                    <Link style={stylesAdd} ><FontIcon className="material-icons" style={iconStylesAdd}
                        
                        onClick={() => { this.handleAddProduct() }}>add</FontIcon><span ></span></Link>
                    : null}
